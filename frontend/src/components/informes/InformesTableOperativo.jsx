        ...
        <thead className="table-light text-uppercase text-muted" style={{ fontSize: "0.7rem", letterSpacing: "0.05em" }}>
          <tr>
            <th style={{ width: "40px" }} className="ps-3 py-3 text-center">
              <Form.Check type="checkbox" checked={allSelected} ref={headerCheckboxRef} onChange={onSelectAll} />
            </th>
            <th style={{ width: "80px" }} className="py-3">
              {renderSortLabel("ID", "id_informe")}
            </th>
            <th style={{ width: "150px" }} className="py-3">
              {renderSortLabel("Plantilla", "nombre_plantilla")}
            </th>
            <th style={{ width: "220px" }} className="py-3">
              {renderSortLabel("Identificador", "titulo")}
            </th>

            {visiblePreguntas.map((q) => (
              <th key={q.id_pregunta} style={{ width: "150px" }} className="py-3">
                {q.etiqueta}
              </th>
            ))}

            <th style={{ width: "140px" }} className="py-3">
              {renderSortLabel("Fecha", "fecha")}
            </th>
            <th style={{ width: "160px" }} className="py-3">
              {renderSortLabel("Usuario", "creado_por")}
            </th>
            <th style={{ width: "240px" }} className="text-center pe-3 py-3">
              Acciones
            </th>
          </tr>
        </thead>
            ...
            return (
              <tr key={inf.id_informe} className={selectedIds.includes(rowId) ? "table-active" : ""}>
                <td className="ps-3 align-middle text-center py-2">
                  <Form.Check type="checkbox" checked={selectedIds.includes(rowId)} onChange={() => onToggleSelection(rowId)} />
                </td>
                ...
                <td className="text-muted align-middle py-2">{formatearFecha(inf.fecha_creado)}</td>
