...
function emptyState(keys) {
  const obj = {};
  keys.forEach((k) => (obj[k] = { ok: false, obs: "" }));
  return obj;
}
...
async function ensureEtapas(idExpediente) {
  ...
  // ✅ FIX: detecta keys faltantes o mal formadas (ok no boolean)
...
exports.create = async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO ema.expedientes
     (id_proyecto, fecha_relevamiento, gps, tecnico, codigo_exp, propietario_nombre, propietario_ci, tramo, subtramo, id_tramo, id_sub_tramo, codigo_censo)
...
exports.update = async (req, res) => {
  ...
  const { rows } = await pool.query(
    `UPDATE ema.expedientes SET
      fecha_relevamiento=$1, gps=$2, tecnico=$3,
      codigo_exp=$4, propietario_nombre=$5, propietario_ci=$6,
      tramo=$7, subtramo=$8, id_tramo=$9, id_sub_tramo=$10, codigo_censo=$11, updated_at=now()
...
exports.setEtapa = async (req, res) => {
  ...
  if (!list.includes(key)) return res.status(400).json({ message: "key inválida para ese tipo" });
...
exports.subirDBI = async (req, res) => {
  ...
  if (!isExpressFileUploadFile(file)) return res.status(400).json({ message: "Archivo inválido" });
