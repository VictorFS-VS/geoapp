 ...
 * notifica (consultor + usuarios del cliente),
...
async function jobContratosVencidos() {
  ...
  for (const c of contratosVencidos) {
    ...
    // 1) Notificar consultor (una vez, dedupe)
    if (id_consultor) {
    ...
    // 2) Notificar cliente (a todos los users activos del id_cliente)
    if (id_cliente) {
    ...
    // 4) Cancelar alertas operativas pendientes (visitas/entregas/auditorías)
    await pool.query(
      `
