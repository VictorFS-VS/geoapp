...
// â CRUD expedientes + Documentos (tumba) + IMPORT EXCEL + â Elaboraci?n de Carpetas (Mejora/Terreno) + â DBI
// â Plano georeferenciado: sube pol?gono (SHP triad / ZIP / KML / KMZ / GeoJSON) usando /api/mantenimiento/upload/:idProyecto (fieldName="files")
// â Inserta en ema.bloque_mejoras / ema.bloque_terreno + pasa id_expediente
// â REGLA NUEVA: â SOLO marca etapa "plano_georef" si el backend confirma ok=true y inserted>0
// â Soporta: .shp+.dbf+.shx (seleccionados juntos), .zip, .kml, .kmz (y opcional .geojson/.json)
// â REGLA: si est?s en "terreno" el nombre debe contener TERRENO; si est?s en "mejora" debe contener MEJORA/MEJORAS
// â Preview muestra si hay SHP incompletos y si el nombre NO cumple la regla del tipo
// â Importaci?n Excel con mapeo de URLs p?blicas de im?genes para CI propietario / adicional
// â Vista previa peque?a de im?genes importadas
...
/* =========================
   â Helpers pol?gono (Mantenimiento-like)
   ========================= */
...
/* =========================
   â Regla: nombre debe incluir TERRENO o MEJORA(S)
   ========================= */
...
export default function Expedientes() {
  ...
  const loadPlanoGeo = async (idExpediente, tipo, tipoPoligono, force = false) => {
    if (!idExpediente) return;
    const cached = planoGeoCache?.[idExpediente]?.[tipo]?.[tipoPoligono];
    if (!force && cached !== null && cached !== undefined) return cached;
    planoGeoLoadingRef.current += 1;
    setPlanoGeoLoading(true);
    const key = `${idExpediente}:${tipo}:${tipoPoligono}`;
    const seq = ++planoGeoReqSeqRef.current;
    planoGeoReqKeyRef.current[key] = seq;
    try {
      const path = tipo === "mejora" ? "mejoras" : "terreno";
      const data = await apiGet(
        `${API}/expedientes/${idExpediente}/${path}?tipo_poligono=${encodeURIComponent(tipoPoligono)}`
      );
      ...
      setPlanoGeoCache((prev) => ({
        ...
        [idExpediente]: {
          ...(prev[idExpediente] || {}),
          [tipo]: {
            ...((prev[idExpediente] || {})[tipo] || { proyecto: null, afectacion: null }),
            [tipoPoligono]: feats,
          },
        },
      }));
  ...
  useEffect(() => {
    ...
    loadPlanoGeo(current.id_expediente, tipoCarpeta, "afectacion");
  ...
  const planoFeaturesProyecto = planoGeoCache?.[current?.id_expediente]?.[tipoCarpeta]?.proyecto;
  const planoFeaturesAfectacion = planoGeoCache?.[current?.id_expediente]?.[tipoCarpeta]?.afectacion;
  const planoHasGeomProyecto = Array.isArray(planoFeaturesProyecto) && planoFeaturesProyecto.length > 0;
  const planoHasGeomAfectacion = Array.isArray(planoFeaturesAfectacion) && planoFeaturesAfectacion.length > 0;
  const planoFeatureCountProyecto = Array.isArray(planoFeaturesProyecto) ? planoFeaturesProyecto.length : 0;
  const planoFeatureCountAfectacion = Array.isArray(planoFeaturesAfectacion) ? planoFeaturesAfectacion.length : 0;
  ...
  const handleVerPlanoEnMapa = async (tipoPoligono) => {
    ...
    const feats = await loadPlanoGeo(current.id_expediente, tipoCarpeta, tipoPoligono);
    const geom = getPolygonGeometry(feats);
    ...
    try {
      ...
      setPlanoGeoCache((prev) => ({
        ...
        [current.id_expediente]: {
          ...
          [tipoCarpeta]: {
            ...((prev[current.id_expediente] || {})[tipoCarpeta] || { proyecto: null, afectacion: null }),
            [tipoPoligono]: [],
          },
        },
      }));
  ...
  // =========================
  // â PARTE 2: Elaboraci?n de carpetas
  // =========================
  const ETAPAS_MEJORA = [
  ...
  /* =========================
     â Sets detectados (como Mantenimiento)
     ========================= */
  ...
  const EXP_FIELDS = [
    ...
    // â NUEVOS: documentos generales desde Excel
  ...
  function guessMapping(headers) {
    ...
    return {
      ...
      tecnico: pick(
        "Datos_Relevamiento_T?cnico_Encargado",
      ...
      codigo_exp: pick(
        "Datos_del_Expediente_C?digo_Expediente_Notificaci?n",
        "Datos_del_Expediente_Codigo_Expediente_Notificacion",
        "codigo expediente notificacion",
        "c?digo expediente notificaci?n",
        "codigo exp",
        "codigo expediente"
      ),

      codigo_censo: pick(
        "Datos_del_Expediente_C?digo_Censo_socioecon?mico_carga_posterior",
        "Datos_del_Expediente_Codigo_Censo_socioeconomico_carga_posterior",
        "codigo censo socioeconomico carga posterior",
        "c?digo censo socioecon?mico carga posterior",
        "codigo censo"
      ),
      ...
      propietario_ci: pick(
        "Datos_del_Expediente_N?mero_C.I._Propietario",
        "Datos_del_Expediente_Numero_C_I_Propietario",
        "numero ci propietario",
        "n?mero ci propietario",
        "propietario ci",
        "cedula propietario"
      ),
  ...
  const save = async () => {
    ...
    const payload = {
      ...form,
      documentacion_presentada: normalizeDocList(form.documentacion_presentada),
      porcentaje_afectacion: porcentajeAfectacionCalc || "",
      desafectado_detalle: form.desafectado
  ...
  const sortedRows = useMemo(() => {
    ...
    const getValue = (r) => {
      switch (sortKey) {
        ...
        case "codigo_exp":
          return String(r.codigo_exp || "").toLowerCase();
        case "propietario_nombre":
        ...
        <Table bordered hover size="sm" className="align-middle" style={{ minWidth: 980 }}>
          <thead>
            <tr>
              ...
              <th role="button" style={{ cursor: "pointer" }} onClick={() => toggleSort("id_expediente")}>
                ID{sortIndicator("id_expediente")}
              </th>
              <th role="button" style={{ cursor: "pointer" }} onClick={() => toggleSort("codigo_exp")}>
                Código Exp.{sortIndicator("codigo_exp")}
              </th>
